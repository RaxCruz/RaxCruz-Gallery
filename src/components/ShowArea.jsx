...
function ShowArea() {
  ...
  return (
    <section class="py-10  bg-gray-100 w-[calc(100%-240px)] float-right min-h-screen">
      <div class="container max-w-6xl mx-auto">
        <h2 class="text-4xl font-bold tracking-tight text-center">
          {collection.title}
        </h2>
        <p class="mt-2 text-lg text-center text-gray-600">
          Check out awesome collection below 😎
        </p>
        <div class="grid grid-cols-4 gap-8 mt-10 sm:grid-cols-8 lg:grid-cols-12 sm:px-8 xl:px-0">
          {collection.contents.map((item) => (
            <div class="relative flex flex-col items-center space-y-3 col-span-4  bg-gray-100 sspace-y-4 overflow-hidden ">
              <div class="">
                <img
                  class="object-contain h-48 w-full rounded-xl"
                  src={item.image}
                  alt=""
                />
              </div>
